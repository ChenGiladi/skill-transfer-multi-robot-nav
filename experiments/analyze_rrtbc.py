...
def perm_test(a, b, n=10000, seed=0):
    """One-sided permutation p-value for mean(a) > mean(b) (deterministic)."""
    a = np.asarray(a, float); b = np.asarray(b, float)
    obs = a.mean() - b.mean()
...
N_SEEDS = df.seed.nunique()   # per-condition seed count; table captions update automatically
...
if len(st):
    ...
    for n in sorted(st.n_agents.unique()):
        ...
        try:
            ...
            p = float("nan")
        pp = perm_test(o.collisions, a.collisions)
        out.append(f"- {n} agents: original coll {ms(o.collisions)} (per-task {o.coll_per_task.mean():.2f}, "
                   ...
                   f"adaptation cuts collisions {red:.0f}% (MWU original>alt p={p:.3g}, perm p={pp:.3g}).")
...
if _INSTR:
    ...
    pl = [r"% AUTO-GENERATED by analyze_rrtbc.py -- do not edit by hand.",
          r"\begin{table}[H]",
          r"\caption{Planning cost on Map2 (" + str(N_SEEDS) + r" seeds, 2000 steps). Runtime is mean\,$\pm$\,SD "
    ...
    cc = [r"% AUTO-GENERATED by analyze_rrtbc.py -- do not edit by hand.",
          r"\begin{table}[H]",
          r"\caption{Collision composition on Map2 at all fleet sizes (" + str(N_SEEDS) + r" seeds, 2000 steps): "
...
if len(st):
    lines = [
        ...
        r"\begin{table}[H]",
        r"\caption{Full skill-transfer results on Map2 (" + str(N_SEEDS) + r" seeds, 2000 scheduler steps). "
        ...
        r"(one-sided, naive\,$>$\,method), the one-sided permutation $p_{\mathrm{perm}}$ "
        r"($10^{4}$ resamples), and Cliff's $\delta$ are computed against that same "
    ...
    for n in sorted(st.n_agents.unique()):
        ...
        for label, ds, kind in method_rows:
            ...
            if ds == "original":
                ...
                pp = perm_test(o.collisions, g.collisions)
                ppstr = "$<\\!0.001$" if pp < 0.001 else f"${pp:.3f}$"
...
_perm = {}       # (map, fraction) -> one-sided permutation p (reviewer item: permutation tests)
_arms = {}       # (map, fraction) -> (none-arm collisions, sharing-arm collisions) for power analysis
for mp in ("map1", "map2"):
    ab = df[df.exp == f"S1_ablation_{mp}"]
    if not len(ab):
        continue
    ...
    for p in (0.6, 1.0):
        ...
        if len(g) and len(none):
            ...
            _perm[(mp, p)] = perm_test(none.collisions, g.collisions)
            _arms[(mp, p)] = (none.collisions.values, g.collisions.values)
...
for (mp, p, pv), hp in zip(_ab_tests, _holm_adj):
    out.append(f"- {mp} share={p}: nominal p={pv:.3g}, perm p={_perm[(mp, p)]:.3g}, Holm p={hp:.3g}, "
               f"{'sig' if hp < 0.05 else 'n.s.'} after correction.")

# ---- Post-hoc power analysis (simulation-based; reviewer item: power analysis) ----
# For each ablation comparison, resample both arms with replacement (2000 iterations) and
# count how often the one-sided Mann-Whitney test detects the effect at alpha=0.05.
out.append("\n## Post-hoc power analysis (bootstrap-simulated Mann-Whitney, 2000 iterations)")
_POWER_IT = 2000
_pname = {0.6: "selective", 1.0: "full"}
for (mp, p, _pv) in _ab_tests:
    a_none, a_share = _arms[(mp, p)]
    rng = np.random.default_rng(1)
    cnt = 0
    for _ in range(_POWER_IT):
        ra = rng.choice(a_none, len(a_none), replace=True)
        rb = rng.choice(a_share, len(a_share), replace=True)
        try:
            if mannwhitneyu(ra, rb, alternative="greater").pvalue < 0.05:
                cnt += 1
        except ValueError:
            pass
    out.append(f"- {mp} {_pname[p]}={p}: post-hoc power at n={len(a_none)} per arm = {cnt/_POWER_IT:.2f}")
# Minimal detectable effect for the primary (Map1 full) comparison: synthesize a k-fraction
# collision reduction with the none arm's dispersion and find the smallest k with >=0.80 power.
if ("map1", 1.0) in _arms:
    a_none = _arms[("map1", 1.0)][0]
    _mde = None
    for k in [i / 100 for i in range(5, 55, 5)]:
        rng = np.random.default_rng(1)
        cnt = 0
        for _ in range(_POWER_IT):
            ra = rng.choice(a_none, len(a_none), replace=True)
            rb = rng.choice(a_none, len(a_none), replace=True) * (1 - k)
            try:
                if mannwhitneyu(ra, rb, alternative="greater").pvalue < 0.05:
                    cnt += 1
            except ValueError:
                pass
        if cnt / _POWER_IT >= 0.80:
            _mde = k
            break
    out.append(f"- map1 full: minimal detectable collision reduction with >=0.80 power at "
               f"n={len(a_none)} per arm = "
               + (f"{100*_mde:.0f}% of the no-sharing mean."
                  if _mde is not None else "not reached within the scanned 5-50% range."))

sl = [
    r"% AUTO-GENERATED by analyze_rrtbc.py -- do not edit by hand.",
    r"\begin{table}[H]",
    r"\caption{Collision-history sharing on the native route library of each map (five "
    r"robots, " + str(N_SEEDS) + r" seeds, 2000 steps). One-sided permutation tests "
    r"($p_{\mathrm{perm}}$, 10\,000 resamples) are reported alongside the Mann--Whitney "
    r"$U$ tests. Only Map1 full participation ($\qshare=1$) is "
    r"nominally significant, and it does not survive correction.}",
    ...
    r"\textbf{Tasks} & \textbf{Change} & \boldmath$p$ & \boldmath$p_{\mathrm{perm}}$ & "
    r"\boldmath$p_{\mathrm{Holm}}$ & \textbf{Significant} \\",
    r"\midrule",
]
for mp in ("map1", "map2"):
    ab = df[df.exp == f"S1_ablation_{mp}"]
    ...
    for k, p in enumerate((0.0, 0.6, 1.0)):
        ...
        if p == 0.0:
            ...
            pv = _fmtp(_nom[(mp, p)]); pvp = _fmtp(_perm[(mp, p)]); pvh = _fmtp(_holm[(mp, p)])
        ...
        sl.append(f"{mapcell} & {p:.1f} & ${g.collisions.mean():.0f}\\pm{g.collisions.std(ddof=1):.0f}$ & "
                  f"{g.coll_per_task.mean():.2f} & {g.tasks_completed.mean():.0f} & {chgs} & {pv} & {pvp} & {pvh} & {sig} \\\\")
